...
usando a WASM do
[Firecrawl anydoc](https://www.npmjs.com/package/@firecrawl/anydoc-wasm).
Diferente dos servidores Go deste repositório, roda via Deno (não gera `.exe` no
`Taskfile`).
...
OpenDocument (`.odt`/`.ods`/`.odp`), RTF, EPUB, CSV e PDF. No Excel, o preview é
limitado a 100 colunas e 50 linhas.
...
Via Taskfile (na raiz deste repositório de MCPs) — gera `../dist/anydoc.exe` (a
tarefa `prep` cria `dist/` antes):

```powershell
task build:anydoc
```

Direto (o binário vai para `../dist/anydoc.exe` — crie a pasta `dist/` antes se
não existir):
